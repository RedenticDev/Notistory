...
@implementation NTSLinkTableViewCell
...
- (void)linkCellWithText:(NSString *)text URL:(NSURL *)url {
	...
	[self setAccessoryViewWithImage:[UIImage imageNamed:@"globe"]];
	self.url = url;
}
...
- (void)twitterCellWithName:(NSString *)name profile:(NSString *)profile {
	...
	[self setAccessoryViewWithImage:[UIImage imageNamed:@"twitter"]];
	...
	/* // Cephei method, outdated
	 [self.activityIndicator startAnimating];
	NSURL *requestURL = [NSURL URLWithString:[NSString stringWithFormat:@"https://mobile.twitter.com/%@/profile_image?size=%@", profile, [UIScreen mainScreen].scale > 2 ? @"original" : @"bigger"]];
	...
	[[[NSURLSession sharedSession] dataTaskWithRequest:request completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
		dispatch_async(dispatch_get_main_queue(), ^{
			...
			NSLog(@"[Notistory] Error loading Twitter profile picture (%@): %ld %@ - %@", requestURL, (long)statusCode, [NSHTTPURLResponse localizedStringForStatusCode:statusCode], error);
			}
		});
	}] resume];*/
